...
function CreateSessionModal({ isOpen, onClose, onCreate }: CreateSessionModalProps) {
  ...
  useEffect(() => {
    if (formData.protocol === 'serial' && isOpen) {
      loadSerialPorts();
    }
  }, [formData.protocol, isOpen]);

  const loadSerialPorts = async () => {
    try {
      const { invoke } = await import('@tauri-apps/api/core');
      const ports = await invoke<string[]>('get_serial_ports');
      setAvailablePorts(ports.length > 0 ? ports : ['COM1', 'COM2', 'COM3', 'COM4']);
      if (ports.length > 0 && !ports.includes(formData.serialPort)) {
        setFormData(prev => ({ ...prev, serialPort: ports[0] }));
      }
    } catch (error) {
  ...
  const handleBrowseKeyFile = async () => {
    try {
      ...
      const selected = await open({
        multiple: false,
        filters: [{ name: '密钥文件', extensions: ['pem', 'key', 'pub', 'ppk'] }],
      });
      if (selected && typeof selected === 'string') {
        setFormData(prev => ({ ...prev, sshKeyPath: selected }));
      }
    } catch (error) {
